package org.mental_management_center.mmc.service; // Перевір свій package

import lombok.RequiredArgsConstructor;
import org.mental_management_center.mmc.model.Article;
import org.mental_management_center.mmc.model.CategoryTranslation; // ДОДАНО
...
public class DataInitializer implements CommandLineRunner {
    ...
    private final CategoryTranslationRepository categoryTranslationRepository; // ДОДАНО ІН'ЄКЦІЮ
    ...
    public void run(String... args) {
        ...
        // 2. ІНІЦІАЛІЗАЦІЯ СЛОВНИКА КАТЕГОРІЙ (Заповнюємо нашу нову таблицю)
        initCategoryDictionary();

        // 3. Створення статтей (всі 7 тем)
    ...
    // НОВИЙ МЕТОД: Заповнює словник перекладів базовими значеннями
    private void initCategoryDictionary() {
        ...
        defaultCategories.forEach((slug, nameUa) -> {
            if (!categoryTranslationRepository.existsById(slug)) {
                categoryTranslationRepository.save(new CategoryTranslation(slug, nameUa));
            }
        });
    }
    ...
    private void saveNewArticle(
            ...
            String content,
            Set<String> tags,
            String imagePath) { // <-- ДОДАЛИ ПАРАМЕТР сюди
        ...
        Article article = Article.builder()
                ...
                .tags(tags)
                .imagePath(imagePath) // <-- ДОДАЛИ В БУДІВНИК ЛОМБОКА
